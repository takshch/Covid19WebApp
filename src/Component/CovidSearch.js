...
export default class CovidSearch extends React.Component{
    constructor(props){
        super(props);
        let tempData = this.props.data.sort((a,b)=>{
    ...
    addData = (val) => {
        ...
        if(val.top === 1 ){
            ...
            if(this.state.searchTempData.length > range){
                let data = this.state.searchTempData.slice(0, this.state.listIndex + 10); 
    ...
    searchData = (e) => {
        if(e.target.value !== ""){
            ...
            // console.log(keyword);
            this.setState({searchStatus: "Loading..."});
            let data = this.state.staticData.filter(function(value){
            ...
            this.setState({searchTempData: tempData, tempData: tempData.slice(0,15),searchStatus: ""});
        }
    }
    ...
    render(){
        let jxsit = this.state.tempData.map((val)=>{
            return (<li>
                ...
                <span>{String(val.TotalConfirmed).replace(/(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g, '$1,')}</span>
            </li>)
        });
        return (<React.Fragment>
                <SearchWrapper>
                    <span>World Report</span>
                    <SearchBoxInput type="text" placeholder="Search here..."  onChange={this.searchData}  />
                    <SearchHeading>
                        <span>Country</span>
                        <span>Cases</span>
                    </SearchHeading>
                    <SearchCountryWrapper id="SearchCountryList">
                        <Scrollbars renderThumbVertical={props => <div {...props} className="ScrollbarsCustom-Thumb"/>}
                          onUpdate={this.addData}>
                        <SearchCountryList>
                            {console.log("listIndex: ",this.state.listIndex)}
                           {this.state.searchStatus !== "Record Not Found" ? jxsit : ""}
                           {this.state.searchStatus !== "" ? <div className="text-center">{this.state.searchStatus}</div> : ""}
                        </SearchCountryList>
                        </Scrollbars>
